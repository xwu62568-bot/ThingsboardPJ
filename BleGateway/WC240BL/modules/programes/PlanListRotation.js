...
class PlanListRotationScreen extends React.Component {
    ...
    initData(){
        ...
        for(i in program.how_long){
            ...
            howLongAll += element[site]
        }
        this.state.planList = this.calcOneSite(parameter,times,howLongAll,seasonDiffOn,skipDays)
        this.grouping()
    }
    calcOneSite(parameter,times,howLong,seasonDiffOn,skipDays){
        switch(parameter.repeat_mode){
            case Func.commonFunc.repeat_mode_even_odd:
                ...
                return this.calcEvenOddMode(isEven,times,howLong,seasonDiffOn,skipDays)
            case Func.commonFunc.repeat_mode_week:
                let weekDays = this.weekDaysToArray(parameter.weekdays)
                return this.calcWeekDayMode(weekDays,times,howLong,seasonDiffOn,skipDays)
            case Func.commonFunc.repeat_mode_interval:
                let interval = parameter.interval_days
                return this.calcIntervalMode(interval,times,howLong,seasonDiffOn,skipDays)
        }
        return []
    }
     /**
     ...
     */
     calcWeekDayMode(weekDays,formatedTimes,howLong,seasonDiffOn,skipDays){
        let planList = []
        ...
        while(dayCount <= limitDay && startMilliseconds <= endMoment.valueOf()){
            ...
            if(skipDays.indexOf(weekday) == -1 && weekDays.indexOf(weekday) >= 0){
                ...
                let diff = ""
                if(seasonDiffOn){
                    const seasonAdjust = this.seasonAdjust(startMoment.month(),howLong)
                    adjustedHowLong = seasonAdjust.howLong
                    diff = seasonAdjust.diff + "%"
                }
     ...
     * @param skipDays 跳过周几 [ 6,0] 周六 周日
     */
    calcIntervalMode(interval,formatedTimes,howLong,seasonDiffOn,skipDays){
     ...
     * @param skipDays 跳过周几 [ 6,0] 周六 周日
     */
    calcEvenOddMode(isEven,formatedTimes,howLong,seasonDiffOn,skipDays){
        let planList = []
    ...
    seasonAdjust(month,howLong){
        ...
        if(adjustMode == 0){
            ...
            //按月
            if(!this.adjustArray){
                this.adjustArray = JSON.parse(this.getSeasonMonth())
            }
            if(this.adjustArray.length == 12){
                if(!isNaN(this.adjustArray[month])){
                    diff = this.adjustArray[month]
                }
            }
        }
        return {
            howLong : howLong + parseInt(howLong * diff / 100),
            diff 
        }
    }
    getCurrentPragram(){
    ...
    canSync(){
        ...
        return !this.state.hasTimeOverlap
            ...
            <View style={{ flex: 1 }}>
                ...
                <SafeAreaView style={{flex:1, backgroundColor: constants.colors.lightGray}}>
                    <TouchableOpacity
                        ...
                        underlayColor='none'>
                        <View style={[styles.mainItem, {marginBottom:10,backgroundColor:"#00000000"}]}>
